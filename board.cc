...
pong_board::pong_board(int w, int h)
    ...
    this->board_mutex = new std::recursive_mutex[(width + 2)*(height + 2)];
    //board_mutex[(y+1)*width + x +1] for cell (x,y)
}
...
int pong_ball::move() {
    //claim all 9 adjacent squares
    std::scoped_lock guard(board.board_mutex[(y + 1)*board.width + x + 1],
                           board.board_mutex[(y + 1 + 1)*board.width + x + 1],
                           board.board_mutex[(y + 1)*board.width + x + 1 + 1],
                           board.board_mutex[(y + 1 + 1)*board.width + x + 1 + 1],
                           board.board_mutex[(y + 1 - 1)*board.width + x + 1],
                           board.board_mutex[(y + 1)*board.width + x + 1 - 1],
                            board.board_mutex[(y + 1 - 1)*board.width + x + 1 + 1],
                           board.board_mutex[(y + 1 - 1)*board.width + x + 1 - 1],
    ...
    if (this->unwarp){
        pong_cell& cnext = this->board.cell(this->x + this->dx,
                                            this->y + this->dy);
        assert(cnext.type == cell_warp);
        std::scoped_lock wguard(board.board_mutex[cnext.warp->x +1 + board.width * (cnext.warp->y +1)]);
        this->x = cnext.warp->x;
        this->y = cnext.warp->y;
        this->stopped = false;
        cnext.warp->board.cell(this->x, this->y).ball = this;
        return 0;
    }
    ...
    if (cnext.ball) {
        ...
        cnext.ball->stoppedm.lock();
        cnext.ball->stopped = false;
        cnext.ball->stoppedm.unlock();
        cnext.ball->stoppedcv.notify_all();
